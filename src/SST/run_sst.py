...
from attribution import compute_joint_attention, get_flow_relevance_for_all_layers, _compute_rollout_attention
...
if __name__ == "__main__":
    ...
    params = torch.load('sst2-3layer-model.pt', map_location=torch.device(device))
    ...
    for flip_case in ['generate', 'pruning']:
        ...
        for case, random_order in [('random', True),
            ...
            for x in test_data_loader:
                ...
                attention_mask = torch.tensor(np.float32(x['attention_mask']), requires_grad=True).unsqueeze(
                    0).long().to(device)
                ...
                if case == 'random':
                    attribution = np.random.normal(0, 1,
                                                   np.array(words).shape)  # np.random.normal(np.array(words).shape)
