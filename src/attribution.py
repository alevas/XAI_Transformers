...
def get_flow_relevance(full_att_mat, input_tokens, layer):
    ...
    flow_values = compute_node_flow(res_G, res_labels_to_index, input_nodes, output_nodes,
                                    length=full_att_mat.shape[-1])

    n_layers = full_att_mat.shape[0]
    length = full_att_mat.shape[-1]
    final_layer_attention_raw = flow_values[(layer + 1) * length: (layer + 2) * length,
                                layer * length: (layer + 1) * length]
...
def get_flow_relevance_for_all_layers(encoded, x, tokens, layers, pad_token):
    ...
    for l in layers:
        ...
        attention_relevance = np.concatenate((attention_relevance, np.array([-1.] * (sen_len - valid_token_len))),
                                             axis=None)
